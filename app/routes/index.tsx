...
export default function Home() {
  const { brand, hero, products, features, stats, testimonials, faq, footer } = siteData;
  ...
  return (
    <div className="min-h-screen bg-white text-[#1a1a1a] selection:bg-[#1a1a1a] selection:text-white font-sans overflow-x-hidden">
      ...
      <nav className="absolute top-0 left-0 right-0 z-[100] flex items-center justify-between px-4 py-3.5 sm:px-5 sm:py-4 md:px-7 md:py-4 lg:px-14 lg:py-5.5 transition-all duration-300">
        <a href="/" className="flex items-center gap-2 text-[#1a1a1a] shrink-0 hover:opacity-80 transition-opacity">
          <div className="w-[34px] h-[34px] bg-[#1a1a1a] text-white rounded-md flex items-center justify-center text-lg shadow-sm">
            {brand.logo.icon}
          </div>
          <span className="text-[17px] font-bold leading-tight tracking-[-0.3px]">{brand.name}</span>
        </a>
        ...
        <ul className="hidden md:flex items-center gap-6 list-none bg-white backdrop-blur-md px-6 py-2.5 rounded-full border border-white/40 shadow-sm">
          {["Products", "Features", "Customers", "FAQ"].map((item) => (
            <li key={item}>
              <a href={`#${item.toLowerCase()}`} className="text-[#1a1a1a] text-[13.5px] font-medium hover:opacity-60 transition-opacity">
                {item}
              </a>
            </li>
          ))}
        ...
        <button className="md:hidden p-1.5 text-[#1a1a1a] ml-auto hover:bg-white/20 rounded-md transition-colors" aria-label="Open menu">
        ...
        <a href="#contact" className="hidden md:inline-flex bg-[#1a1a1a] text-white px-[22px] py-2.5 rounded-full text-[13px] font-medium hover:bg-[#333] hover:-translate-y-[1px] transition-all shadow-md shrink-0 whitespace-nowrap">
          {hero.ctas[0].label}
        </a>
      </nav>

      {/* ==================== HERO ==================== */}
      <section className="relative w-full min-h-[500px] lg:min-h-[640px] overflow-hidden flex flex-col justify-end" id="hero">
        ...
        <div className="relative z-10 w-full max-w-[1300px] mx-auto px-4 sm:px-5 md:px-7 lg:px-14 pt-[100px] pb-8 md:pb-12 lg:pb-16 flex flex-col justify-end min-h-[500px] lg:min-h-[640px]">
          ...
          >
            ...
            <div className="flex items-center gap-2 sm:gap-3 text-[12px] sm:text-[13px] text-[#555] whitespace-nowrap shrink-0 self-end sm:self-auto bg-white/30 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20 shadow-sm">
              <span className="font-bold text-[#1a1a1a]">{stats[3].value} / 5</span>
              <div className="flex gap-[1px] text-[#d42b2b]">
                {[...Array(5)].map((_, i) => (
                  <span key={i}>★</span>
                ))}
              </div>
              <span className="text-[#666] font-medium hidden sm:inline">{hero.socialProof}</span>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <motion.section
        ...
        variants={stagger}
      >
        <div className="grid grid-cols-2 md:grid-cols-4 max-w-[1000px] mx-auto" id="stats">
          {stats.map((s, i) => (
            <motion.div 
            ...
            >
              ...
              <span className="text-[13px] text-[#777] mt-1 text-center font-medium uppercase tracking-wider">{s.label}</span>
            </motion.div>
          ))}
        </div>
      </motion.section>
      ...
      {/* ==================== PRODUCTS ==================== */}
      <section className="bg-[#f8f8fb] px-4 py-16 sm:px-5 sm:py-20 md:px-7 md:py-24 lg:px-14 lg:py-32" id="products">
        <div className="max-w-[1100px] mx-auto">
          <motion.div
            ...
            custom={0}
          >
            <span className="inline-block text-[11px] font-bold uppercase tracking-[2.5px] text-[#666] mb-4 bg-white px-3 py-1 rounded-full border border-[#e5e5e5] shadow-sm">Products</span>
            <h2 className="text-[32px] md:text-[42px] font-normal tracking-[-0.5px] text-[#1a1a1a] mb-4">Tools that run your business</h2>
            <p className="text-[15px] text-[#666] max-w-[500px] mx-auto leading-[1.6]">
              Three powerful products. One unified platform. Built for teams that move fast and hate friction.
            </p>
          </motion.div>
          ...
          >
            {products.map((product) => (
              ...
              >
                ...
                <h3 className="text-[22px] font-bold text-[#1a1a1a] tracking-[-0.3px] mb-1">{product.name}</h3>
                <p className="text-[14px] text-[#888] font-medium mb-4">{product.tagline}</p>
                <p className="text-[14.5px] leading-[1.6] text-[#555] mb-7">{product.description}</p>
                  ...
                  {product.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className="text-[13.5px] text-[#444] leading-[1.5]">
                        <strong className="text-[#1a1a1a] font-semibold">{f.title}</strong> — {f.description}
                      </span>
                    </li>
                  ))}
                </ul>
      ...
      {/* ==================== FEATURES ==================== */}
      <section className="bg-white px-4 py-16 sm:px-5 sm:py-20 md:px-7 md:py-24 lg:px-14 lg:py-32" id="features">
        <div className="max-w-[1100px] mx-auto">
          <motion.div
            ...
            custom={0}
          >
            <span className="inline-block text-[11px] font-bold uppercase tracking-[2.5px] text-[#666] mb-4 bg-[#f4f4f6] px-3 py-1 rounded-full shadow-inner">Why {brand.name}</span>
            <h2 className="text-[32px] md:text-[42px] font-normal tracking-[-0.5px] text-[#1a1a1a] mb-4">Built different. Built better.</h2>
            <p className="text-[15px] text-[#666] max-w-[540px] mx-auto leading-[1.6]">
              Every detail is obsessed over so you can focus on what matters — growing your business.
            </p>
          </motion.div>
          ...
          >
            {features.map((feat, i) => (
              ...
              >
                ...
                <h3 className="text-[17px] font-bold text-[#1a1a1a] mb-2">{feat.title}</h3>
                <p className="text-[14px] leading-[1.6] text-[#777]">{feat.description}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      {/* ==================== TESTIMONIALS ==================== */}
      <section className="bg-[#f0f0f4] px-4 py-16 sm:px-5 sm:py-20 md:px-7 md:py-24 lg:px-14 lg:py-32" id="testimonials">
        <div className="max-w-[1100px] mx-auto">
          <motion.div
            className="text-center mb-16 flex flex-col items-center"
            initial="hidden"
            ...
            custom={0}
          >
            <span className="inline-block text-[11px] font-bold uppercase tracking-[2.5px] text-[#666] mb-4">Testimonials</span>
            <h2 className="text-[32px] md:text-[42px] font-normal tracking-[-0.5px] text-[#1a1a1a]">Loved by teams everywhere</h2>
          </motion.div>
            ...
            {testimonials.map((t, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 pt-5 border-t border-[#f0f0f0]">
                  ...
                  <div>
                    <div className="text-[14.5px] font-bold text-[#1a1a1a]">{t.author}</div>
      ...
      {/* ==================== FAQ ==================== */}
      <section className="bg-white px-4 py-16 sm:px-5 sm:py-20 md:px-7 md:py-24 lg:px-14 lg:py-32" id="faq">
        <div className="max-w-[700px] mx-auto">
          <motion.div
            ...
            custom={0}
          >
            <span className="inline-block text-[11px] font-bold uppercase tracking-[2.5px] text-[#666] mb-4">FAQ</span>
            <h2 className="text-[32px] md:text-[42px] font-normal tracking-[-0.5px] text-[#1a1a1a]">Questions? Answers.</h2>
          </motion.div>
            ...
            {faq.map((item, i) => (
              ...
              >
                ...
                >
                  ...
                  <div className={`w-8 h-8 rounded-full bg-[#f4f4f6] flex items-center justify-center shrink-0 transition-transform duration-300 ${openFaq === i ? 'rotate-180 bg-[#1a1a1a] text-white' : 'text-[#888]'}`}>
                ...
                >
                  <p className="px-5 lg:px-6 pb-6 text-[14px] leading-[1.7] text-[#666]">{item.answer}</p>
                </motion.div>
              </motion.div>
            ))}
          </motion.div>
      ...
      >
        <div className="max-w-[640px] mx-auto bg-white/30 backdrop-blur-md p-10 lg:p-14 rounded-3xl border border-white/40 shadow-xl">
          <h2 className="text-[36px] md:text-[42px] font-normal text-[#1a1a1a] tracking-[-0.5px] leading-[1.1] mb-5">Ready to move faster?</h2>
          <p className="text-[15px] sm:text-[16px] text-[#444] mb-10 leading-[1.6]">
            Join <strong>{stats[0].value}{stats[0].suffix} businesses</strong> already using {brand.name}. Start free — no credit card required.
          </p>
      ...
      {/* ==================== FOOTER ==================== */}
      <footer className="bg-[#111] text-white border-t-4 border-[#1a1a1a]" id="site-footer">
        <motion.div
        ...
        >
          {/* Brand column */}
          <motion.div className="flex flex-col md:col-span-2 lg:col-span-1 lg:pr-6" variants={fadeUp}>
            <a href="/" className="flex items-center gap-3 text-white mb-5 hover:opacity-80 transition-opacity">
              <span className="w-8 h-8 bg-white text-[#1a1a1a] rounded flex items-center justify-center text-lg shadow-md">{brand.logo.icon}</span>
              <span className="text-[18px] font-bold tracking-[-0.3px]">{brand.name}</span>
            </a>
            <p className="text-[14px] leading-[1.65] text-white/50 mb-7">Software that scales with you. No chaos, just growth.</p>
          ...
          {footer.columns.map((col) => (
            <motion.div className="flex flex-col" key={col.title} variants={fadeUp}>
              <h4 className="text-[12px] font-bold uppercase tracking-[1.5px] text-white/40 mb-5">{col.title}</h4>
              <ul className="flex flex-col gap-3.5 list-none">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-[14px] text-white/70 hover:text-white hover:translate-x-1 block transition-all">
                      {link.label}
                    </a>
                  </li>
                ))}
          ...
          <motion.div className="flex flex-col" variants={fadeUp}>
            <h4 className="text-[12px] font-bold uppercase tracking-[1.5px] text-white/40 mb-5">Contact</h4>
            <ul className="flex flex-col gap-4 list-none">
              <li className="flex items-center gap-3 text-[14px]">
                ...
                <a href={`mailto:${footer.contact.email}`} className="text-white/70 hover:text-white transition-colors">{footer.contact.email}</a>
              </li>
              <li className="flex items-center gap-3 text-[14px]">
                <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-white/60">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-6 md:px-10 lg:px-14 max-w-[1300px] mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-5 sm:gap-6">
            {footer.legalLinks.map((link) => (
              <a key={link.label} href={link.href} className="text-[13px] text-white/30 hover:text-white/70 transition-colors">
                {link.label}
              </a>
            ))}
          </div>
